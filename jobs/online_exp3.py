print("__WTI_TAG__", "online_exp3.py", "hotfix")
import argparse
import math
...
def find_latest_price_file() -> Path:
    ...
    if not files:
        raise SystemExit("æ‰¾ä¸åˆ°è³‡æ–™æª”ï¼šcapital_wti_downloader/output/OIL_CRUDE_HOUR_*.csv")
    return files[-1]
...
def load_price_returns(path: Path) -> pd.Series:
    ...
    for candidate in ("snapshotTimeUTC", "snapshotTime", "ts", "time"):
        if candidate in df.columns:
            ...
            break
    if time_col is None:
        raise SystemExit(f"åƒ¹æ ¼æª”æ¡ˆç¼ºå°‘æ™‚é–“æ¬„ä½: {path}")
    ...
    if price_col is None:
        if {"close_bid", "close_ask"}.issubset(df.columns):
            ...
            raise SystemExit(f"åƒ¹æ ¼æª”æ¡ˆç¼ºå°‘ close æ¬„ä½: {path}")
...
def load_indexed_csv(path: Path, columns: Optional[Sequence[str]] = None) -> pd.DataFrame:
    if not path.exists():
        raise SystemExit(f"æ‰¾ä¸åˆ°è³‡æ–™æª”ï¼š{path}")
    df = pd.read_csv(path)
    if "ts" not in df.columns:
        raise SystemExit(f"{path} ç¼ºå°‘ ts æ¬„ä½")
...
def main() -> None:
    ...
    if len(common_index) == 0:
        raise SystemExit("è³‡æ–™æ™‚é–“ç¯„åœç„¡äº¤é›†ï¼Œç„¡æ³•è¨ˆç®—")
    ...
    if args.all:
        ...
        if len(common_index) == 0:
            raise SystemExit("æŒ‡å®šå€é–“å…§ç„¡è³‡æ–™")
...
# === HOTFIX: ensure news/term/crack columns exist ===
needed_cols = ["news","term","crack"]
for c in needed_cols:
    if c not in signals.columns:
        signals[c] = 0.0
    # === HOTFIX ensure three arms exist ===
for _col in ("news","term","crack"):
    if _col not in signals.columns:
        signals[_col] = 0.0
else:
    pass
pass
    ...
    pass
    pass
    arm_matrix = signals[["news","term","crack"]].to_numpy()
else:
    pass
pass
    pass
    ...
    print(
        f"Turnover |Î”w_t| mean (7D)={turnover_7d:.6f}  overall={turnover_mean:.6f}",
    )
