...
# 2) features_hourly_with_term???? WTI + term/crack/OVX + GDELT?
FEAT_PATH = ROOT / "data" / "features_hourly_with_term.parquet"
if not FEAT_PATH.exists():
    raise FileNotFoundError(f"??? {FEAT_PATH}???? features_hourly_with_term.parquet ?? data/ ???")
...
for c in ["ts_utc", "ts", "timestamp"]:
    if c in df.columns:
        ...
        break
if ts_col is None:
    raise RuntimeError("features ????????(ts_utc/ts/timestamp)")
...
if len(df) == 0:
    raise RuntimeError("????????????? features_hourly_with_term.parquet ??????")
...
if ret_col is None:
    raise RuntimeError("features ??? ret_1h / wti_returns ??")
...
print("")
print("=" * 80)
print("CLBZ FULL BACKTEST RESULT (2024-10 ~ 2025-12, features_hourly_with_term)")
print("=" * 80)
